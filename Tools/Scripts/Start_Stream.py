...
def build_gst_cmd(laptop_ip: str, port: int = None) -> list:
    ...
    pipeline = (
        ...
        "x264enc tune=zerolatency speed-preset=ultrafast bitrate=25000 key-int-max=15 ! "
...
def unregister_stream_process(proc: subprocess.Popen) -> None:
    """Remove a stream from the global list after the caller has stopped it (avoids double-terminate)."""
    with _stream_lock:
        try:
...
def stop_all_streams() -> None:
    """Terminate every registered UDP stream (Ctrl+C / process exit safety net)."""
    with _stream_lock:
